...
trainset, testset = get_dataset(num_workers=0, batch_size=25, augment_data=False, device=device)
# odevae = ODEVAE(order=1, q=4, n_filt=4).to(device)
odevae = INVODEVAE(order=1, q=2, n_filt=4, qs=5).to(device)
...
for ep in range(Nepoch):
    for i,local_batch in enumerate(trainset):
        ...
        Xrec_tr, qz0_m, qz0_v, zt_tr, lhood, kl_z = odevae(tr_batch, len(trainset), method='rk4')
        tr_loss = -lhood + kl_z
        tr_loss.backward()
        optimizer.step()
        # print('Iter:{:<2d} lhood:{:8.2f}  kl_z:{:<8.2f} '.format(i, lhood.item(), kl_z.item()))
    with torch.no_grad():
        Xrec_tr, qz0_m, qz0_v, zt_tr, lhood, kl_z = odevae(tr_batch, len(trainset), method='rk4', Tode=2*tr_batch.shape[1])
        for test_batch in testset:
            test_batch = test_batch.to(device)
            Xrec_test, qz0_m, qz0_v, zt_test, lhood, kl_z = odevae(test_batch, len(test_batch), method='rk4', Tode=2*test_batch.shape[1])
